...
func findVoxelModelAsset(entityId EntityId, cmd *Commands, server *AssetServer) (voxModelId AssetId, asset *VoxelModelAsset, paletteId AssetId, palette *VoxelPaletteAsset) {
	assetIdType := reflect.TypeOf(AssetId(""))
	allComponents := cmd.GetAllComponents(entityId)
	for _, c := range allComponents {
		val := reflect.ValueOf(c)
	...
	return voxModelId, asset, paletteId, palette
}
