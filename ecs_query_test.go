...
func TestQuery_Without(t *testing.T) {
	...
	ecs := MakeEcs()
	ecs.addEntity(Comp1{a: 1})                          // comp1 only                       -- shouldn't match (needs Comp2)
	id2 := ecs.addEntity(Comp1{a: 2}, Comp2{b: 1.37})   // comp1 & comp2                    -- should match
	ecs.addEntity(Comp1{a: 3}, Comp2{b: 4.20}, Comp3{}) // comp1 & comp2 & comp3            -- shouldn't match (has Comp3)
	ecs.addEntity(Comp1{a: 4}, Comp3{})                 // comp1 & comp3                    -- shouldn't match
	ecs.addEntity(Comp2{b: 3.14})                       // comp2 only                       -- shouldn't match

	query := Query2[Comp1, Comp2]{ecs: &ecs}.Without(Type[Comp3]())

	expectedA := map[EntityId]Comp1{id2: {a: 2}}
	numResults := 0
