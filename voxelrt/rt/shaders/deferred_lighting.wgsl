...
fn sample_point_shadow(
    light: Light,
    shadow_receiver_pos: vec3<f32>,
    shadow_normal: vec3<f32>,
    L: vec3<f32>,
    receiver_shadow_group_id: u32,
    receiver_shadow_seam_epsilon: f32
) -> f32 {
    let light_to_receiver = shadow_receiver_pos - light.position.xyz;
    ...
    let biasM = max(seam_tolerance_m, 0.05 + texel_depth_m + voxel_shadow_bias(0.08, NdL));
            ...
            if (light_type == 1u) {
                ...
                if (pos_ls.w > 0.0 && shadow_uv.x >= 0.0 && shadow_uv.x <= 1.0 && shadow_uv.y >= 0.0 && shadow_uv.y <= 1.0) {
                    ...
                    if (!voxel_shadow_skip_grazing(NdL)) {
                        ...
                        let baseBiasM = 0.05;
                        let slopeBiasM = 0.1;
                        let biasM = baseBiasM + voxel_shadow_bias(slopeBiasM, NdL);
