...
struct CelestialBody {
    ...
    flags: vec4<f32>,  // x disable_surface
};
...
fn sample_surface(
    ...
    let terrain = saturate(continental * 0.72 + ridge * 0.4 - basin * 0.14);

    let lowland_color = base_color * vec3<f32>(0.7, 0.9, 1.08);
    let highland_color = base_color * vec3<f32>(1.08, 1.03, 0.92);
    let peak_color = mix(base_color, vec3<f32>(1.0, 1.0, 1.0), 0.45);
    ...
    let wrapped_diffuse = saturate((dot(normal, light_dir) + 0.28) / 1.28);
    let half_vec = normalize(light_dir + view_dir);
    let spec = pow(saturate(dot(normal, half_vec)), 48.0) * (0.02 + 0.06 * ridge);
    let fresnel = pow(1.0 - NdotV, 5.0);
    let night_fill = vec3<f32>(0.025, 0.03, 0.045) * (1.0 - NdotL) * 0.8;

    return albedo * (0.16 + wrapped_diffuse * 0.96) + spec + fresnel * 0.04 + night_fill;
}
...
fn sample_clouds(
    ...
    let lighting = 0.28 + NdotL * 0.78 + limb * 0.24;
    let color = body.cloud_color.xyz * lighting * self_shadow;
    let alpha = mask * cloud_opacity * (0.12 + 0.18 * NdotL + 0.12 * limb);
...
fn sample_atmosphere(
    ...
    let day_side = saturate(dot(shell_normal, light_dir) * 0.5 + 0.5);
    ...
    let scatter_color = (base_color * (0.09 + rayleigh * 0.16 + mie * 0.045 + day_side * 0.36 + edge_glow * 0.78 * atmosphere_glow) + warm_scatter) * (0.42 + density_soft * 0.58);
    let alpha = saturate(optical_depth * (density_soft * 0.24 + edge_glow * 0.42 * atmosphere_glow) * (0.18 + 0.2 * day_side + mie * 0.04));
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    var best_t = 1e9;
    var best_surface_t = -1.0;
    var best_atmo_entry = 1e9;
    var best_atmo_exit = -1.0;
    ...
    for (var i = 0u; i < body_params.body_count; i++) {
        ...
        let atmo_exit = atmo_hit.y;
        var surface_t = -1.0;
        if (surface_hit.y >= 0.0) {
            surface_t = max(surface_hit.x, 0.0);
        }

        if (atmo_entry < best_t) {
            best_t = atmo_entry;
            best_index = i32(i);
            best_surface_t = surface_t;
            best_atmo_entry = atmo_entry;
            best_atmo_exit = atmo_exit;
        }
    }
    ...
    let body = bodies[u32(best_index)];
    let center = body.center_radius.xyz;
    let radius = body.center_radius.w;
    let atmo_radius = max(body.params.x, radius);
    ...
    if (opaque_t < 50000.0 && opaque_t <= best_atmo_entry) {
        return vec4<f32>(0.0);
    }

    var atmosphere_cap = best_atmo_exit;
    if (opaque_t < 50000.0) {
        atmosphere_cap = min(atmosphere_cap, opaque_t);
    }
    let atmosphere = sample_atmosphere(body, ray_dir, body_light_dir, best_atmo_entry, best_atmo_exit, atmosphere_cap, radius, atmo_radius);
    var depth_cap = best_atmo_exit;
    if (opaque_t < 50000.0) {
    ...
    let surface_occluded = opaque_t < 50000.0 && (best_surface_t < 0.0 || opaque_t < best_surface_t);
    if (best_surface_t < 0.0 || disable_surface || surface_occluded) {
        ...
        return vec4<f32>(cloud_mix, cloud_alpha);
    }

    let hit_pos = ray_origin + ray_dir * best_surface_t;
    ...
    var surface_color = sample_surface(body, normal, surface_light_dir, surface_view_dir);

    if (front_clouds.a > 0.0) {
        surface_color *= 1.0 - front_clouds.a * 0.16;
        surface_color = mix(surface_color, front_clouds.xyz, front_clouds.a);
    }

    let shell_to_surface = max(best_surface_t - best_atmo_entry, 0.0);
    ...
    let atmo_mix = saturate(atmosphere.a * 0.75 + haze * 0.4 + horizon * 0.34);
    let final_color = mix(surface_color + emission, surface_color * 0.88 + atmosphere.xyz + emission, atmo_mix);
    return vec4<f32>(saturate3(final_color), 1.0);
}
