...
struct CameraData {
  ...
  pad2: vec2<f32>,
  ao_quality: vec4<f32>, // x: AO sample count, y: AO radius, zw: reserved shadow style controls
...
@group(1) @binding(13) var<storage, read> voxel_aux_words : array<u32>;
...
fn decode_baked_voxel_normal(encoded: u32) -> BakedVoxelNormal {
  ...
  return BakedVoxelNormal(
    normalize(n),
    true,
    (encoded & VOXEL_NORMAL_TWO_SIDED_BIT) != 0u,
  );
}
...
fn load_baked_voxel_normal_local(vi: vec3<i32>, params: ObjectParams) -> BakedVoxelNormal {
  ...
  let sz = vi.z >> 5u;
  let sector_idx = find_sector_cached(sx, sy, sz, params);
  if (sector_idx < 0) { return BakedVoxelNormal(vec3<f32>(0.0), false, false); }
  let sector = sectors[sector_idx];
  let bx = (vi.x >> 3u) & 3;
  let by = (vi.y >> 3u) & 3;
  let bz = (vi.z >> 3u) & 3;
...
fn sample_directional_shadow(
  ...
  return select(0.0, 1.0, hard_seam_lit || hard_sampled_depth_n >= my_depth_n - bias);
}
...
fn calculate_light_contribution(
  ...
  if (light.shadow_meta.y > 0u) {
    if (light_type == 1u) {
      ...
      attenuation *= sample_directional_shadow(light, p, shadow_normal, L, receiver_shadow_group_id, receiver_shadow_seam_epsilon, selection.primary_index);
