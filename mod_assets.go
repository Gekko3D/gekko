...
func createVolumeTexels(voxModel *VoxModel) []uint8 {
	volume := make([]uint8, voxModel.SizeX*voxModel.SizeY*voxModel.SizeZ*4)
	for _, v := range voxModel.Voxels {
		idx := (int32(v.Z)*voxModel.SizeX*voxModel.SizeY + int32(v.Y)*voxModel.SizeY + int32(v.X)) * 4
		color := voxModel.Palette[v.ColorIndex]
...
func (server AssetServer) LoadVoxelBasedTexture(voxModel *VoxModel) AssetId {
	volumeTexels := createVolumeTexels(voxModel)
	return server.CreateTexture(volumeTexels[:], uint32(voxModel.SizeX), uint32(voxModel.SizeY), uint32(voxModel.SizeZ), TextureDimension3D, TextureFormatRGBA8Uint)
}
