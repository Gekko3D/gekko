...
- Text and gizmo ECS bridge sync are gated by registered graph feature ownership and are installed as registered bridge systems outside the broad `voxelRtSystem` flow. Text sync uses a tested `buildTextBridgeItems` adapter to hand `TextOverlayItem` input to the renderer feature, and gizmo sync uses `buildGizmoBridgeItems` to hand `GizmoOverlayItem` input to the renderer feature.
